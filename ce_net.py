...
def create_macro(G, macro_mapping, macro_types={}):
    ...
    weights = np.array(list(macro_mapping.items()))

    unique_micro = np.unique(weights[:, 0])
    unique_macro, weights[:, 1] = np.unique(weights[:, 1], return_inverse=True)

    mapping_mat = np.zeros((len(unique_micro), len(unique_macro)))
    mapping_mat[weights[:, 0], weights[:, 1]] = 1

    M = mapping_mat.T @ Wout_micro @ mapping_mat
    M_row_sum = M.sum(axis=1, keepdims=True)
    M_normalized = M / M_row_sum
    ...
    #
    # if macro_types == {}:
    #     macro_types = {j:'spatial' for i, j in macro_mapping.items() if i != j}
    #
...
def causal_emergence(G, span=-1, thresh=1e-4, t=500,
    ...
    EI_current = EI_micro/np.log2(G_micro.number_of_nodes())    #########
    ...
    for node_i in micro_nodes_left:
        ...
        while len(queue) > 0:
            ...
            EI_macro = EI_macro/np.log2(G_macro.number_of_nodes())  #########
